...
// Root is bit 0.
uint8_t buttonStates[6] = {0, 0, 0, 0, 0, 0};
const int chordBankSizes[4] = {8, 7, 7, 8}; // Maj, Min, 7/9, Mixed

const char *chordLabels[4][10] = {
    {"Maj", "Maj7", "Maj9", "Maj13", "Sus4", "Sus2", "Maj6", "Inv1", "",
     ""}, // Maj Bank
    {"Min", "Min7", "Min9", "Min11", "Dim", "m7b5", "Inv1", "", "",
     ""}, // Min Bank
    {"3rd", "5th", "7th", "9th", "11th", "13th", "Open", "", "",
     ""}, // 7/9 Bank
    {"Aug", "Aug7", "Dim7", "DimM7", "Sus4", "Sus2", "Inv1", "Inv2", "",
     ""} // Mixed
};

// Bitmasks (0=C, 1=C#, etc.)
const uint16_t chordMasks[4][10] = {
    // Btn 1: Maj Bank
    {(1 << 0) | (1 << 4) | (1 << 7),                        // Maj
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 11),            // Maj7
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 11) | (1 << 2), // Maj9
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 11) | (1 << 9), // Maj13
     (1 << 0) | (1 << 5) | (1 << 7),                        // Sus4
     (1 << 0) | (1 << 2) | (1 << 7),                        // Sus2
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 9),             // Maj6
     (1 << 4) | (1 << 7) | (1 << 12)},                      // Maj Inv1

    // Btn 2: Min Bank
    {(1 << 0) | (1 << 3) | (1 << 7),                        // Min
     (1 << 0) | (1 << 3) | (1 << 7) | (1 << 10),            // Min7
     (1 << 0) | (1 << 3) | (1 << 7) | (1 << 10) | (1 << 2), // Min9
     (1 << 0) | (1 << 3) | (1 << 7) | (1 << 10) | (1 << 5), // Min11
     (1 << 0) | (1 << 3) | (1 << 6),                        // Dim
     (1 << 0) | (1 << 3) | (1 << 6) | (1 << 10),            // m7b5
     (1 << 3) | (1 << 7) | (1 << 12)},                      // Min Inv1

    // Btn 3: 7/9 Bank
    {(1 << 0) | (1 << 4),                                   // 3rd
     (1 << 0) | (1 << 7),                                   // 5th
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 10),            // Dominant 7th
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 10) | (1 << 2), // Dominant 9th
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 10) | (1 << 5), // Dominant 11th
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 10) | (1 << 9), // Dominant 13th
     (1 << 0) | (1 << 7) | (1 << 12)},                      // Open (1-5-8)

    // Btn 4: Mixed Technical
    {(1 << 0) | (1 << 4) | (1 << 8),             // Aug
     (1 << 0) | (1 << 4) | (1 << 8) | (1 << 10), // Aug7
     (1 << 0) | (1 << 3) | (1 << 6) | (1 << 9),  // Dim7
     (1 << 0) | (1 << 3) | (1 << 6) | (1 << 11), // DimMaj7
     (1 << 0) | (1 << 5) | (1 << 7),             // Sus4 (Dup for easy access)
     (1 << 0) | (1 << 2) | (1 << 7),             // Sus2 (Dup)
     (1 << 5) | (1 << 7) | (1 << 12),            // Sus4 Inv1
     (1 << 2) | (1 << 7) | (1 << 12)}            // Sus2 Inv1
};
...
void updateActiveNotes() {
  ...
  int btnState =
      (activeButtonIndex >= 1 && activeButtonIndex <= 4) // Changed from 6 to 4
          ? buttonStates[activeButtonIndex]
          : 0;
  int invCutoff = 0;
  // Inversions are now at specific indices in the chord banks, not a generic
  // state 2-4 We need to check the specific chord type for inversion logic
  if (activeButtonIndex >= 1 && activeButtonIndex <= 4) {
    // For now, let's assume inversions are the last few states in each bank
    // This will need more sophisticated logic if inversions are mixed in
    // For the current chordMasks, Inv1 is the last element in Maj/Min/7/9
    // banks, Inv1/Inv2 in Mixed This logic needs to be tied to the actual chord
    // definition, not just state. For now, we'll simplify and assume inversion
    // states are at the end of the bank. This is a placeholder and might need
    // refinement based on how inversions are truly defined in chordMasks. The
    // current chordMasks have specific inversion entries, so the
    // getInversionCutoff is still relevant but the 'btnState >= 2 && btnState
    // <= 4' condition is no longer generic for inversions. Instead, we need to
    // check if the current 'state' (btnState) corresponds to an inversion
    // chord. For the provided chordMasks, 'Inv1' is at index 7 for Maj/Min, and
    // 'Inv1', 'Inv2' at 6,7 for Mixed. This means the 'btnState' itself
    // directly indicates the chord, including inversions. So, the
    // 'getInversionCutoff' should be called if the selected chord *is* an
    // inversion. This requires knowing which chord states are inversions. For
    // simplicity, let's assume the last few states in each bank are inversions.
    // This is a temporary simplification.
    if (activeButtonIndex == 1 && btnState == 7) { // Maj Inv1
      invCutoff = getInversionCutoff(currentChordMask, 1);
    } else if (activeButtonIndex == 2 && btnState == 6) { // Min Inv1
      invCutoff = getInversionCutoff(currentChordMask, 1);
    } else if (activeButtonIndex == 3 &&
               btnState == 6) { // Open (1-5-8) - not an inversion, but a
                                // specific voicing
      // No inversion logic for this one based on the mask
    } else if (activeButtonIndex == 4 && btnState == 6) { // Mixed Inv1
      invCutoff = getInversionCutoff(currentChordMask, 1);
    } else if (activeButtonIndex == 4 && btnState == 7) { // Mixed Inv2
      invCutoff = getInversionCutoff(currentChordMask, 2);
    }
  }
...
void drawCalibrationScreen(int step) {
  ...
  if (step == 1) {
    ...
    tft.drawLine(0, 0, 25, 0, TFT_WHITE);
    tft.drawLine(0, 0, 0, 25, TFT_WHITE);
  } else if (step == 2) {
    ...
    tft.drawLine(SCREEN_WIDTH - 25, SCREEN_HEIGHT - 1, SCREEN_WIDTH - 1,
                 SCREEN_HEIGHT - 1, TFT_WHITE);
    tft.drawLine(SCREEN_WIDTH - 1, SCREEN_HEIGHT - 25, SCREEN_WIDTH - 1,
                 SCREEN_HEIGHT - 1, TFT_WHITE);
  } else if (step == 3) {
  ...
  for (int i = 0; i < btnCount; i++) {
    ...
    if (i == 0) { // Octave Down
      ...
      if (activeButtonIndex == -1) {
        ...
        int s = buttonStates[i];
        label = chordLabels[i - 1][s];
        // Cycle colors based on state
        uint16_t stateColors[] = {TFT_GREEN,   TFT_CYAN,   TFT_YELLOW,
                                  TFT_MAGENTA, TFT_ORANGE, TFT_PURPLE,
                                  TFT_OLIVE,   TFT_NAVY,   TFT_DARKGREY};
        color = stateColors[s % 9];
        textColor = TFT_BLACK;
      } else {
...
void handleButtonPress(int index) {
  // Index 0-5 (Octave/Chords)
  if (index == 0) { // Octave Down
    octaveShift--;
    if (octaveShift < -3)
      octaveShift = -3;
  } else if (index == 5) { // Octave Up
    octaveShift++;
    if (octaveShift > 3)
      octaveShift = 3;
  } else {
    ...
    if (activeButtonIndex == index) {
      // Cycle State
      buttonStates[index] =
          (buttonStates[index] + 1) % chordBankSizes[index - 1];
    } else {
      if (activeButtonIndex != -1) {
        activeButtonIndex = -1;
...
int getVisualStringOffset() {
  ...
  int targetCenterIndex =
      36; // C4 (Middle C) is index 36 in baseFreqs (Starts C1)
...
void drawBootScreen() {
  ...
  tft.setTextSize(1);
  tft.setTextDatum(TC_DATUM);
  tft.drawString("v1.3", SCREEN_WIDTH / 2,
...
void fireArp(int octaveOffset, bool latched) {
  ...
  // Safety: If Latched, release PREVIOUS Latched voices immediately
  if (latched) {
    for (int v = 0; v < MAX_VOICES; v++) {
      if (voices[v].active && voices[v].isLatchedArp) {
        voices[v].release();
  ...
  if (latched) {
    // Find Voice
    int vIdx = -1;
    // 1. Try to find Empty Slot (IF and ONLY IF we are under the Governor
    // Limit)
    int activeCount = 0;
    for (int v = 0; v < MAX_VOICES; v++) {
      if (voices[v].active)
    ...
    // 2. Stealing Logic (Priority based)
    if (vIdx == -1) {
      // Priority A: Steal Released (Not Held) & Unlatched
      for (int v = 0; v < MAX_VOICES; v++) {
        if (voices[v].active && !voices[v].held && !voices[v].isLatchedArp) {
    ...
    // Priority C: Desperation (Steal 0)
    if (vIdx == -1)
      vIdx = 0;
    ...
    int effectiveOctave = latched ? (latchedOctaveShift + 1) : octaveShift;
...
void loop() {
  ...
  if (audioTarget == TARGET_SPEAKER || audioTarget == TARGET_BLUETOOTH ||
    ...
    if (ts.touched()) {
      ...
      // Performance Zone
      if (ty < 55) { // Sound Design Bar (Active < 55, Dead 55-60)
        static uint32_t lastTopPress = 0;
        if (millis() - lastTopPress > 300) {
          ...
          } else if (topIdx == 5) { // Wave/Editor
            if (wavePressStart == 0)
              wavePressStart = millis();

            // Reliable Long Press Detection (v1.3)
            if (millis() - wavePressStart > 600) {
              currentMode = MODE_EDIT;
              editorInputBlocked = true;
              tft.fillScreen(COLOR_BG);
              drawEditor();
              wavePressStart = 0; // Reset so release doesn't cycle wave
              delay(500);
              return;
            }
            drawWaveButton(); // Visual feedback for press
          }
        }
      } else if (ty >= 193) { // Presets (Active >= 193, Dead 187-193)
        int btnW = SCREEN_WIDTH / 6;
        int btnIdx = -1;
        // Horizontal Dead Zone (3px border)
        int localX = tx % btnW;
        if (localX >= 3 && localX <= btnW - 3) {
          btnIdx = tx / btnW;
        }
        ...
        if (btnIdx != -1 && millis() - lastBtnPress > 300) {
          ...
          lastBtnPress = millis();
                 ...
                 ty < 187) { // Sliders (Active 141-187, Dead 138-141 & 187-193)
        if (tx < 240) {
          float volNormalized = (float)tx / 240.0f;
      ...
      } else if (ty >= 63 && ty < 138) { // Strings (Active 63-138)
        int numStrings = activeParams.octaveRange * 12 + 1;
