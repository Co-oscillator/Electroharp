...
void SynthVoice::trigger(float freq, int noteIdx, Waveform wave, float pw,
  ...
  frequency = freq;
  phase = 0.0f;
  phaseIncrement = freq / (float)currentSampleRate;
  waveform = wave;
  pulseWidth = pw;
  ...
  attackRate = 1.0f / (attack * (float)currentSampleRate);
  decayRate = 1.0f / (decay * (float)currentSampleRate);
  releaseRate = 1.0f / (release * (float)currentSampleRate);
  sustainLvl = sustain;
}
...
float IRAM_ATTR SynthVoice::getSample(float pitchMod, float pwMod) {
  ...
  if (waveform == WAVE_SAW) {
    ...
    if (pwMod > 0.01f) {
      float gain = 1.0f + (pwMod * 4.0f); // Boost gain to drive fold (Max 5x)
      raw *= gain;

      // Fast Wavefolding (Unrolled Loop x2 - Handles Gain < 5.0)
      if (raw > 1.0f)
        raw = 2.0f - raw;
      else if (raw < -1.0f)
        raw = -2.0f - raw;
      ...
      // Clamp final just in case gain > 5.0 (Safety)
      if (raw > 1.0f)
        raw = 1.0f;
      if (raw < -1.0f)
        raw = -1.0f;
    ...
    // Wrap index safely using bitwise AND (Fast)
    int idx2 = (idx + 1) & 0xFF;
    idx &= 0xFF;
