...
void updateStringVisuals() {
  ...
  for (int i = 0; i < STRING_COUNT; i++) {
    ...
    if (currentColor != lastStringColor[i]) {
      ...
      int drawY = 40;
      int drawH = 120; // 160 - 40 (Shortened for Taller Volume Slider)
      ...
      if (i % 12 == 0) {
        ...
        tft.drawString(noteNames[rootNote], x + (w / 2),
                       labelY); // Correct Label
      }
...
void drawTransposeButton() {
  ...
  char buf[32]; // Used by sprintf if needed, kept for future use or remove
                // nicely.
  // Actually, we don't use it anymore below.
  // But let's just make it clean.
  // Remove redundant sprintf draw? The user said "Transpose Transpose - C".
  // This implies `tft.drawString(buf...)` printed "Transpose\nC" AND then
  // manual lines printed duplicates. We should just use the manual layout for
  // better control.

  // sprintf(buf, "Transpose\n%s", noteNames[rootNote]);
  // tft.drawString(buf, x + w / 2, y + h / 2); // CAUSED DUPLICATION

  tft.drawString("Transpose", x + w / 2, y + 10);
  tft.drawString(noteNames[rootNote], x + w / 2, y + 28);
}
...
void drawEditor() {
  ...
  drawSliderControl(3 * topW, topY, topW, topH, "LDR Tgt",
                    (float)activeParams.ldrTarget, 0, 8, true);
...
void handleEditorTouch(int tx, int ty) {
  ...
  if (ty < topH) {
    ...
    case 3: // LDR Tgt
    {
      ...
      drawSliderControl(rx, ry, rw, rh, "LDR Tgt", (float)v, 0, 7, true);
      lastTopPress = millis();
    } break;
    }
...
void triggerNote(int sIdx) {
  ...
  if ((currentChordMask & (1 << (sIdx % 12))) && sIdx >= invCutoff) {
    ...
    // Semitones: rootNote. freq * 2^(rootNote/12)
    if (rootNote != 0) {
...
void fireArp(int octaveOffset, bool latched) {
  ...
  if (latched) {
    ...
    voices[vIdx].trigger(freq, stringIndex, currentWaveform, globalPulseWidth,
                         activeParams.attackTime, 0.5f, 0.6f, 2.5f);
...
void loop() {
  static bool waitForArpRelease =
      false; // Prevents Latch from Immediate Release
  ...
  if (millis() - lastLdr > 20) {
    ...
    // Let's do: Dark = BaseFreq, Light = BaseFreq * 2 (+1 Octave)?
    ...
    float targetLfoDepth = activeParams.lfoDepth; // Default
    ...
    } else if (activeParams.ldrTarget == TARGET_LFO_DEPTH) {
      // Dark = 0, Light = Slider
      targetLfoDepth = activeParams.lfoDepth * ldrMod;
    }
    ...
    // 3. Update Global Volatiles (Atomic-ish)
    // Filter
    svf_f = 2.0f * sin(3.14159f * targetCutoff / SAMPLE_RATE);
    // ... (Res is handled in audio thread via activeParams? No, need check)
    // Wait, resMod is local in audio loop. baseRes = activeParams.filterRes *
    // resMod. LDR modulates 'targetRes'. We need to pass targetRes to audio?
    // Audio reads `activeParams.filterRes`.
    // We should update a global `svf_q` or `globalRes`?
    // Line 120: `volatile float svf_q = 0.22f;` exists.
    // Audio line 235: `float baseRes = activeParams.filterRes * resMod;`
    // It ignores `svf_q`?
    // Line 242: `float q = 1.0f - baseRes;`
    // It seems Audio uses activeParams directly.
    // To support LDR Res, we need a global `globalRes` similar to
    // `globalLfoDepth`. But for now, let's fix LFO Depth first.

    globalLfoDepth = targetLfoDepth; // Update Global
    lastLdr = millis();
  }
  ...
  /*
  static uint32_t lastLdrLog = 0;
  if (millis() - lastLdrLog > 2000) { // Heartbeat every 2s
  ...
  */
  ...
  if (ts.touched()) {
    ...
    if (p.z < 400) {
      if (lastTouchedString != -1) {
        // Release ALL held UNLATCHED notes on pressure drop
        for (int v = 0; v < MAX_VOICES; v++) {
    ...
    if (ty < 50) {
      // 6 Buttons: 53px width each.
      // 0: Delay (<53), 1: Drive (<106), 2: Trem (<159), 3: LFO (<212), 4: Arp
      // (<265), 5: Wave
      static uint32_t lastTopPress = 0;
      if (millis() - lastTopPress > 300) {
        ...
        // 5. Arp (< 265)
          ...
          if (!waitForArpRelease) {
            ...
            // Long Press Latch Check
            if (arpPressStart > 0 && millis() - arpPressStart > 500) {
              if (!arpLatch) { // Only trigger latch ONCE per hold
                arpLatch = true;
                latchedOctaveShift = octaveShift; // Lock current octave
                drawArpButton();                  // Yellow
                arpPressStart = 0;                // consume press
                waitForArpRelease = true;         // Prevents restarting timer
              }
            }
          }
          // Reset flag if we released? No, we are INSIDE touch block here.
          // Flag needs to be pushed to scope where Release happens?
          // Static inside this block persists? Yes.
          // Check if NOT touching to clear?
          // That happens in the ELSE block of `if (ts.touched())`.
          // But `waitForArpRelease` is local static here.
          // I need to move it to a broader scope or check `!ts.touched()`
          // inside its scope? No, I can't check !touched here.

          // Better: Use a static at `loop` level or Global.
          // Or reuse a global if available.
          // Let's make it static at the top of the `if (ty < 50)` block?
        }
        // 6. Wave (> 265)
        else {
          // Long Press Activity
          ...
          // Animate Gradient while holding
          drawWaveButton();

          // Check Hold Duration
          if (wavePressStart > 0 && millis() - wavePressStart > 250) {
            // Enter Editor
        ...
        // lastTopPress = millis(); // Don't debounce HOLD actions globally,
        // only Taps.
        // Actually lastTopPress logic is weird here. It blocks repeated entry.
        // If we remove it, we flood Drive/Trem/LFO toggles.
        // We should only update lastTopPress if we TOGGLED something.
        // For Hold (Wave/Arp/Delay), we need continuous entry.
        // Hack: Reset lastTopPress if holding Arp/Wave?
        // Better: Move Toggles inside the check, Leave Holds outside?
        // Existing code did `lastTopPress = millis()` at end.
      }

      // Update toggle debounce
      if (tx < 212 && tx >= 53) { // Drive/Trem/LFO
        // Handled inside
        lastTopPress = millis();
      }
      // Delay/Arp/Wave need continuous check for Hold/Release logic

      // Clear string interaction if touching buttons
      if (lastTouchedString != -1) {
        for (int v = 0; v < MAX_VOICES; v++) {
          if (voices[v].active && voices[v].noteIndex == lastTouchedString &&
              voices[v].held && !voices[v].isLatchedArp)
            voices[v].release();
        }
        lastTouchedString = -1;
      }
    ...
    // 3. Slider Zone (170 - 200) -> Actually Input Logic for Sliders?
    // Wait, Slider Zone IS y > SCREEN - 70? (240 - 70 = 170).
    // Volume Slider is at Y=160, H=40. So 160-200.
    // Buttons are at Y=200, H=40.
    // Logic: if ty > 200 -> Bottom Buttons.
    // if ty > 160 && ty < 200 -> Volume/Transpose.
    else if (ty >= 160 && ty < 200) {

      // Split: 0-240 (Volume), 240-320 (Transpose)
      if (tx < 240) {
        // Volume Slider
        float vol = (float)tx / 240.0f;
        if (vol < 0.0f)
          vol = 0.0f;
        if (vol > 1.0f)
          vol = 1.0f;
        ...
        if (millis() - lastTransPress > 250) {
          ...
          if (rootNote > 11)
            rootNote = 0;
          drawTransposeButton();
          // Redraw Strings to update visual Key
          drawStrings();
          lastTransPress = millis();
        }
      }

      // Clear string interaction for safety
      if (lastTouchedString != -1) {
        for (int v = 0; v < MAX_VOICES; v++) {
          if (voices[v].active && voices[v].noteIndex == lastTouchedString &&
              voices[v].held && !voices[v].isLatchedArp)
    ...
    // 4. Middle Zone: Strings (50 - 170)
      ...
      if (sIdx >= 0 && sIdx < STRING_COUNT) {
        if (sIdx != lastTouchedString) {
          ...
          if (lastTouchedString != -1) {
            // Release held UNLATCHED voices on slide
            for (int v = 0; v < MAX_VOICES; v++) {
              if (voices[v].active && voices[v].held && !voices[v].isLatchedArp)
                voices[v].release();
            }
          ...
          // Trigger New
          // ARP LOGIC INTERCEPTION
          if (arpMode != ARP_OFF && !arpLatch) {
            // Only trigger if Active String
            int invCutoff = 0; // Recalculate or use updateActiveNotes logic?
            // Just check mask directly.
            // getInversionCutoff logic duplicated here or make helper?
            // Let's assume standard mask check for simplicity or reuse
            // `activeNotes`. Actually `activeNotes` array assumes sorted valid
            // buttons. Faster: Check bitmask.
            if (currentChordMask & (1 << (sIdx % 12))) {
              // Calculate Octave Offset (0, 12, 24...)
              int octaveOffset = (sIdx / 12) * 12;
              fireArp(octaveOffset);
            }
          } else {
            triggerNote(sIdx);
          }
  ...
  // Cleanup if no touch
    ...
    waitForArpRelease = false; // Reset Latch Hold prevention
    ...
    // Check for Arp Toggle on Release
    if (arpPressStart > 0) {
      Serial.printf("Arp Release! PStart: %u, Latched: %d\n", arpPressStart,
                    arpLatch);
      // Single Tap Logic (Cycle Mode) - Double Tap Removed

      if (arpLatch) {
        // Immediate OFF from Latch
        arpMode = ARP_OFF;
        arpLatch = false;

        // Explicitly release any Latched Voices immediately
        for (int v = 0; v < MAX_VOICES; v++) {
          if (voices[v].isLatchedArp) {
            voices[v].release();
          }
        ...
        // Normal Cycle OR Latch Fallback (if pressure dropped during Hold)
        if (millis() - arpPressStart > 500) {
          // Treat as Hold -> Latch ON
          arpLatch = true;
          drawArpButton();
          Serial.println("Arp Latch Triggered (Release Fallback)!");
        } else {
          // Short Tap -> Cycle Mode
          int m = (int)arpMode;
          m++;
          if (m > 5)
            m = 0; // Cycle ARP_RANDOM (5) -> OFF (0)
          arpMode = (ArpMode)m;

          // If we just turned ON Random, Shuffle
      ...
      if (arpMode == ARP_OFF) {
        arpLatch = false;
        // Ensure cleanup if cycled to OFF
        for (int v = 0; v < MAX_VOICES; v++) {
          if (voices[v].isLatchedArp) {
            voices[v].release();
          }
      ...
      // Regen Random Sequence on entry
      if (arpMode == ARP_RANDOM) {
        shuffleArpSequence();
        Serial.println("Arp: Random Shuffle");
      }
