...
void drawCalibrationScreen(int step) {
  ...
  tft.drawString("Touch Calibration", SCREEN_WIDTH / 2, 40);

  if (step == 1) {
    tft.drawString("Touch Top Left Corner", SCREEN_WIDTH / 2, 120);
    tft.drawCircle(10, 10, 5, TFT_RED);
  } else if (step == 2) {
    tft.drawString("Touch Bottom Right Corner", SCREEN_WIDTH / 2, 120);
    tft.drawCircle(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 10, 5, TFT_RED);
  } else {
    tft.drawString("COMPLETE", SCREEN_WIDTH / 2, 120);
  }
}
...
void updateStringVisuals() {
  ...
  for (int i = 0; i < STRING_COUNT; i++) {
    ...
    if (currentColor != lastStringColor[i]) {
      // Top Bar: 0-60. Strings: 60 -> 140 (reduced by 20px).
      int drawY = 60;
      int drawH = 140 - 60; // 80px height for strings
      ...
      if (i % 12 == 0) {
        ...
        // Correct labeling: show note for this string index
        int noteNum = (i + rootNote) % 12;
        if (noteNum < 0)
          noteNum += 12;
        tft.drawString(noteNames[noteNum], x + (w / 2), labelY);
      }
...
void setupSpeaker() {
  ...
  // Pin 4 must be LOW to enable. Pins 21, 27, 22 are HIGH for power/backlight.
  pinMode(4, OUTPUT);
...
void drawBootScreen() {
  ...
  // Split Screen
  // Left: Bluetooth
  tft.fillRect(0, 0, SCREEN_WIDTH / 2, SCREEN_HEIGHT - 60, TFT_BLUE);
  tft.setTextColor(TFT_WHITE);
  tft.drawString("BLUETOOTH", SCREEN_WIDTH / 4, (SCREEN_HEIGHT - 60) / 2);

  // Right: Speaker
  tft.fillRect(SCREEN_WIDTH / 2, 0, SCREEN_WIDTH / 2, SCREEN_HEIGHT - 60,
               TFT_DARKGREEN);
  tft.drawString("SPEAKER", 3 * SCREEN_WIDTH / 4, (SCREEN_HEIGHT - 60) / 2);
...
void loadSettings() {
  preferences.begin("harp-cfg", false);

  uint32_t savedVersion = preferences.getUInt("ver", 0);
  if (savedVersion != CONFIG_VERSION) {
    Serial.println("Version Mismatch! Wiping defaults...");
    saveSettings();
    return;
  }

  // LED settings removed
  preferences.end();
}

void saveSettings() {
  preferences.begin("harp-cfg", false);
  preferences.putUInt("ver", CONFIG_VERSION);
  // LED settings removed
  preferences.end();
}

void setup() {
  // --- SHOTGUN PIN INIT (Rev 3) ---
  ...
  // --- STARTUP TONE TEST ---
  // Simple 440Hz Square wave for 200ms to verify hardware
  // This runs BEFORE interrupts/timers start.
  for (int i = 0; i < 88; i++) { // ~88 cycles of 440Hz
    dacWrite(26, 200);           // High
    delayMicroseconds(1136);     // 1/880s
    dacWrite(26, 50);            // Low
    delayMicroseconds(1136);
  }
  dacWrite(26, 128); // Back to mid-rail
...
void triggerNote(int sIdx) {
  float freq = baseFreqs[sIdx]; // Fix missing var
...
void loop() {
  ...
  if (audioTarget == TARGET_BOOT) {
    ...
    static uint32_t failsafeTimer = 0;
    static TS_Point lastValidPoint;
    static bool potentialFailsafe = false;
    ...
    if (ts.touched()) {
      ...
      if (p.z > 200) {
        ...
        if (failsafeTimer == 0) {
          ...
          potentialFailsafe = true;
        }
        if (millis() - failsafeTimer > 3000) {
          audioTarget = TARGET_CALIBRATION_1;
          drawCalibrationScreen(1);
          delay(1000);
          failsafeTimer = 0;
          potentialFailsafe = false;
          return;
        }
      }
      return;
    ...
    if (potentialFailsafe) {
      uint32_t dur = millis() - failsafeTimer;
      failsafeTimer = 0;
      potentialFailsafe = false;
      if (dur > 50 && dur < 1000) {
        int touchX = lastValidPoint.x;
        int touchY = lastValidPoint.y;
        if (touchX < 140) { // Left: BT
          audioTarget = TARGET_BT_SELECT;
          foundDeviceCount = 0;
          setupBluetooth(true);
          drawBTSelectScreen(-1);
        } else if (touchX > 180) { // Right: Speaker
          audioTarget = TARGET_SPEAKER;
          setupSpeaker();
          tft.fillScreen(TFT_BLACK);
          drawInterface();
        } else if (touchY > 180) { // Bottom: Config
  ...
  if (audioTarget == TARGET_SPEAKER || audioTarget == TARGET_BLUETOOTH ||
    ...
    if (millis() - lastLdr > 20) {
      ...
      float ldrNorm = map(constrainedLdr, 0, 2000, 1000, 0) / 1000.0f;
      float ldrMod = constrain(ldrNorm, 0.0f, 1.0f);

      float targetCutoff = activeParams.filterCutoff;
      ...
      float targetLfoDepth = activeParams.lfoDepth;

      if (activeParams.ldrTarget == TARGET_FILTER) {
        float minF = 100.0f;
        targetCutoff =
            minF + (activeParams.filterCutoff - minF) * (ldrMod * ldrMod);
        ...
        targetFold = 0.1f + (activeParams.waveFold - 0.1f) * ldrMod;
      ...
      svf_f = 2.0f * sin(PI * targetCutoff / (float)SAMPLE_RATE);
      if (svf_f > 0.95f)
        svf_f = 0.95f;
      svf_q = 1.0f - targetRes;
      globalPulseWidth = targetFold;
      globalLfoDepth = targetLfoDepth;
    ...
    if (ts.touched()) {
      ...
      if (audioTarget == TARGET_CONFIG) {
        ...
        if (ty > startY + (btnH + gap) && ty < startY + (btnH + gap) + btnH) {
          // Calibration (Assuming placeholder logic or actually implemented
          // elsewhere) For now, let's just ignore or implement if found. In the
          // UI it's MAGENTA.
          return;
        }
