...
bool isBluetoothActive = false; // Track if BT was ever started
...
uint32_t wavePressStart = 0; // For long press
uint32_t arpPressStart = 0;  // For long press
uint32_t delayPressStart = 0;
uint32_t drivePressStart = 0;
uint32_t tremPressStart = 0;
uint32_t lfoPressStart = 0;
...
volatile uint8_t *audioBuffer = NULL; // Heap Allocated for Safety
volatile int bufReadHead = 0;
volatile int bufWriteHead = 0;
volatile uint32_t isrCount = 0;
volatile uint32_t lastFillDuration = 0; // Performance Tracking
...
void releaseLatchedVoices(); // Helper for unlatching
void drawArpButton();
...
// Downsample Factor: 6 for "Lo-Fi" efficiency (Allows 12 voices on BT)
#define DELAY_DOWNSAMPLE 6
...
volatile float delayLpfState = 0.0f;  // For feedback damping
int activeSliderIdx = -1;             // To lock onto a slider during drag

// UI State
int lastTouchedString = -1;
int lastChordBtn = -1; // Global for reset on release

// --- PERFORMANCE GOVERNOR ---
int maxPolyphony = MAX_VOICES;
...
int getHomeOffset() {
  ...
  int rawOffset = 36 - (numStrings / 2);
  int homeOffset = ((rawOffset + 6) / 12) * 12; // Round to nearest octave
  if (homeOffset < 0)
    homeOffset = 0;
  // Ensure we don't start so high that we go out of bounds on the right
  if (homeOffset + numStrings > STRING_COUNT)
    homeOffset = ((STRING_COUNT - numStrings) / 12) * 12;
  return homeOffset;
}

int getGlobalNoteIndex(int sIdx) {
  return sIdx + getHomeOffset() + (octaveShift * 12);
}
...
void releaseLatchedVoices() {
  for (int v = 0; v < MAX_VOICES; v++) {
    if (voices[v].active) {
      voices[v].isLatchedArp = false;
      voices[v].release();
    }
  }
}
...
void updateDerivedParameters() {
  ...
  svf_f = 2.0f * sin(PI * activeParams.filterCutoff / (float)activeSampleRate);
  if (svf_f > 0.95f)
    svf_f = 0.95f;
...
int32_t bt_data_stream_callback(Frame *data, int32_t len) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    float wobble = (sin(wowPhase) + sin(flutterPhase)) *
                   0.0007f; // significantly reduced wobble
    ...
    if (fxLFO) {
      ...
      float depth = activeParams.lfoDepth; // Local or Global? Global is safe.
    ...
    if (delayTick == 0 && delayBuffer != 0) {
      if (delayMode > 0) {
        ...
        // Damping (One-pole LPF ~0.66 coeff)
        delayLpfState = delayLpfState * 0.34f + fb * 0.66f;
        fb = delayLpfState;
  ...
  // --- GOVERNOR LOGIC (Duplicated for BT Context) ---
  // BT callback shouldn't take > 80% of its budget.
  // Budget depends on buffer size (len).
  // len samples @ 44.1k = len * 22.6us.
  // If len=512, budget ~ 11ms.
  uint32_t elapsed = micros() - startT;
  float budget = (float)len * (1000000.0f / 44100.0f);
...
void ARDUINO_ISR_ATTR onTimer() {
  ...
  isrCount++; // DEBUG: Count interrupts
...
// --- Audio Generation Task (Called in loop) ---
// --- Audio Generation Task (Burst Mode) ---
// --- Audio Generation Task (Burst Mode) ---
void fillAudioBuffer() {
  ...
  int freeSpace = (r - w + AUDIO_BUF_SIZE - 1) % AUDIO_BUF_SIZE;

  // 3. Decide Burst Size
  int samplesToFill = 2048;
  if (samplesToFill > freeSpace)
    samplesToFill = freeSpace;

  if (samplesToFill <= 0)
    return;

  // 4. Tight Generation Loop
  for (int i = 0; i < samplesToFill; i++) {
    ...
    if (activeParams.lfoType == LFO_SINE) {
      ...
      lfoVal = ((float)random(1000) / 500.0f) - 1.0f;
    }

    // --- Apply LFO Targets ---
    float pitchMod = 1.0f; // Base multiplier
    float pwMod = 0.0f;
    float filterMod = 0.0f;
    float resMod = 1.0f;

    // effectiveDepth calculated
    float effectiveDepth = activeParams.lfoDepth * globalLfoDepth;

    if (activeParams.lfoTarget == TARGET_PITCH) {
      pitchMod = 1.0f + (lfoVal * effectiveDepth * 0.1f);
    } else if (activeParams.lfoTarget == TARGET_FOLD) {
      if (activeParams.lfoType == LFO_SQUARE)
        pwMod = (lfoVal > 0) ? activeParams.lfoDepth : 0.0f;
      else
        pwMod = lfoVal * effectiveDepth * 0.4f;
    } else if (activeParams.lfoTarget == TARGET_FILTER) {
      filterMod = lfoVal * effectiveDepth * 1000.0f;
    } else if (activeParams.lfoTarget == TARGET_RES) {
      resMod = 1.0f - (lfoVal * effectiveDepth * 0.5f);
    }

    // --- SYNTHESIS CORE ---
    float sample = generateMixedSample(pitchMod, pwMod, filterMod, resMod);
    ...
    if (delayMode > 0 && delayBuffer != 0) {
      int delayMs = delayMode * 300;
      // Fixed: delaySamples should use activeSampleRate / DELAY_DOWNSAMPLE
      int delaySamples = (activeSampleRate / DELAY_DOWNSAMPLE * delayMs) / 1000;
      int readPos = (delayHead - delaySamples + MAX_DELAY_LEN) % MAX_DELAY_LEN;
      delayed = (float)delayBuffer[readPos] * 3.3333e-5f;
    }
    ...
    if (delayTick == 0 && delayBuffer != 0) {
      ...
      delayHead++;
      if (delayHead >= MAX_DELAY_LEN)
        delayHead = 0;
    }

    float totalSample = sample;

    // Final Output Mapping
    int out = 128 + (int)(totalSample * 127.0f);
    if (out < 0)
      out = 0;
    if (out > 255)
      out = 255;
    ...
    // Write to Buffer
    int currentWriteIdx = (w + i) % AUDIO_BUF_SIZE;
    audioBuffer[currentWriteIdx] = (uint8_t)out;
  }

  // 5. Commit Write Head
  bufWriteHead = (w + samplesToFill) % AUDIO_BUF_SIZE;

  lastFillDuration = micros() - startT;

  // Preserve Governor Logic (Calculates Load even if ignored)
  if (millis() - lastLoadCheck > 50) {
    uint32_t elapsed = lastFillDuration;
    float budget = (samplesToFill * 1000000.0f) / activeSampleRate;
    float load = (float)elapsed / budget;
...
// --- AUDIO TASK (High Priority / Core 0) ---
// --- AUDIO TASK (High Priority / Core 0) ---
void audioTask(void *parameter) {
  while (1) {
    if (audioTarget == TARGET_SPEAKER && audioBuffer != NULL) {
      // Hysteresis: Only fill if we have a decent chunk to write (e.g. 128
      // samples) This prevents waking up every 45us to write 1 sample.
      int r = bufReadHead;
      int w = bufWriteHead;
      int freeSpace = (r - w + AUDIO_BUF_SIZE - 1) % AUDIO_BUF_SIZE;
      ...
      if (freeSpace > 128) {
        ...
        // Buffer is nearly full, sleep longer to let ISR consume
        vTaskDelay(5);
      }
    } else {
      vTaskDelay(10);
...
void updateStringVisuals() {
  ...
  for (int i = 0; i < numStrings; i++) {
    ...
    // Playable Range check (v1.3 Refinement)
    float freq = baseFreqs[clampedIdx];
    // Apply Octave Shift to freq estimation for dimming
    int totalTwist = octaveShift;
    if (totalTwist > 0) {
    ...
    if (outOfBounds || freq < 20.0f || freq > 4800.0f)
      baseColor =
          alphaBlend(TFT_BLACK, baseColor, 0.3f); // Dim out-of-bounds strings
  ...
  if (circlesNeedRedraw) {
    ...
    // Fix: Clear leftover space past numStrings if range shrank
    if (numStrings < STRING_COUNT) {
      int xStart =
          (numStrings * SCREEN_WIDTH) / numStrings; // Wait, logic error
      // Actually tft.fillRect in drawStrings handles the clean wipe of the
      // whole area.
    }
  }
}
...
void drawStrings() {
  ...
  // Clear background for range expansion/reduction (y=70, h=100)
  tft.fillRect(0, 70, SCREEN_WIDTH, 100, TFT_BLACK);
...
// Root is bit 0.
uint8_t buttonStates[7] = {
    0}; // Track bank 0-4 for each button (indices 0-1 for Oct, 2-6 for Chords)

const char *chordLabels[5][5] = {
    {"Maj", "Maj7", "Inv 1", "Inv 2", "Inv 3"}, // Btn 1: Majors
    {"Min", "Min7", "Inv 1", "Inv 2", "Inv 3"}, // Btn 2: Minors
    {"7th", "Dim7", "Inv 1", "Inv 2", "Inv 3"}, // Btn 3: 7ths/Dim
    {"9th", "Dom9", "Inv 1", "Inv 2", "Inv 3"}, // Btn 4: Dominant 9ths
    {"Aug/Sus", "Sus4", "Sus2", "Aug7",
     "Aug9"} // Btn 5: Aug/Sus (Updated Label)
};
...
const uint16_t chordMasks[5][5] = {
    ...
    {(1 << 0) | (1 << 4) | (1 << 7) | (1 << 10) | (1 << 2), // 9th
     ...
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 10) | (1 << 2) | (1 << 9), // 13th
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 10) | (1 << 9),            // 7add13
     (1 << 0) | (1 << 4) | (1 << 7) | (1 << 10) | (1 << 1)},           // 7b9
...
void updateActiveNotes() {
  ...
  int btnState = (activeButtonIndex >= 1 && activeButtonIndex <= 5)
                     ? buttonStates[activeButtonIndex]
                     : 0;
  int invCutoff = 0;
  if (activeButtonIndex >= 1 && activeButtonIndex <= 5 && btnState >= 2 &&
      btnState <= 4) {
...
// --- UI HELPERS ---
// Layout: 6 Buttons across top
// [Delay] [Drive] [Trem] [LFO] [Arp] [Wave]

void drawArpButton() {
  int w = SCREEN_WIDTH / 6;
  int x = 4 * w; // Slot 4
  int y = 0;
  int h = 70; // Taller
...
void drawWaveButton() {
  ...
  tft.setTextSize(1);
  tft.setTextDatum(BC_DATUM); // Text at bottom
  const char *txt = "Saw";
  if (currentWaveform == WAVE_SQUARE)
    txt = "Sqr";
  else if (currentWaveform == WAVE_SINE)
    txt = "Sin";
  else if (currentWaveform == WAVE_TRIANGLE)
    txt = "Tri";
  tft.drawString(txt, x + w / 2, y + h - 5);

  tft.setTextDatum(MC_DATUM); // Restore for icon

  // Draw Icon (Centered in button width)
  int iconW = w - 10;
  int x0 = x + (w - iconW) / 2;
  int x1 = x0 + iconW / 4;
  int x2 = x0 + iconW / 2;
  ...
  if (currentWaveform == WAVE_SAW) {
    ...
    for (int i = 0; i <= iconW; i++) {
      float angle = (i / (float)iconW) * 6.28318f; // 2PI
...
// --- DRAW VOLUME SLIDER ---
void drawVolumeSlider() {
  int y = 190; // Centered in 185-235 touch zone
  int h = 40;
  int volW = (SCREEN_WIDTH * 3) / 4;

  tft.fillRect(0, y, volW, h, TFT_BLACK);
  tft.drawRect(0, y, volW, h, TFT_WHITE);

  int fillW = (int)(masterVolume * (volW - 4));
  tft.fillRect(2, y + 2, fillW, h - 4, TFT_CYAN);

  tft.setTextColor(TFT_WHITE);
  tft.setTextDatum(MC_DATUM);
  tft.drawString("VOLUME", volW / 2, y + h / 2);
}

// --- DRAW TRANSPOSE BUTTON ---
void drawTransposeButton() {
  int y = 190; // Centered in 185-235 touch zone
  int h = 40;
  int volW = (SCREEN_WIDTH * 3) / 4;
  int transW = SCREEN_WIDTH - volW;
  int transX = volW;

  tft.fillRect(transX, y, transW, h, TFT_MAGENTA);
  tft.drawRect(transX, y, transW, h, TFT_WHITE);

  tft.setTextColor(TFT_WHITE);
  tft.setTextDatum(MC_DATUM);
  String root = noteNames[rootNote];
  tft.drawString(root, transX + transW / 2, y + h / 2);
}
...
void drawCalibrationScreen(int step) {
  ...
  if (step == 1) {
    tft.drawString("Touch Top Left corner", SCREEN_WIDTH / 2,
                   SCREEN_HEIGHT / 2);
    // Draw Target in Top-Left (0,0)
    tft.fillCircle(0, 0, 15, TFT_RED);
    tft.drawCircle(0, 0, 20, TFT_WHITE);
    tft.drawLine(0, 0, 30, 0, TFT_WHITE);
    tft.drawLine(0, 0, 0, 30, TFT_WHITE);
  } else if (step == 2) {
    tft.drawString("Touch Bottom Right corner", SCREEN_WIDTH / 2,
                   SCREEN_HEIGHT / 2);
    // Draw Target in Bottom-Right
    tft.fillCircle(SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1, 15, TFT_RED);
    tft.drawCircle(SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1, 20, TFT_WHITE);
    tft.drawLine(SCREEN_WIDTH - 30, SCREEN_HEIGHT - 1, SCREEN_WIDTH - 1,
                 SCREEN_HEIGHT - 1, TFT_WHITE);
    tft.drawLine(SCREEN_WIDTH - 1, SCREEN_HEIGHT - 30, SCREEN_WIDTH - 1,
                 SCREEN_HEIGHT - 1, TFT_WHITE);
  } else if (step == 3) {
...
void drawConfigMenu() {
  ...
  tft.setTextSize(1);
  tft.setTextDatum(MC_DATUM);
  tft.drawString("AUDIO CONFIG (exp)", btnX + btnW / 2, startY + btnH / 2);
...
void updateButtonVisuals() {
  ...
  int y = 250; // Aligned with ty >= 250
  int h = 70;  // Fills to bottom (320)

  // 40 / 80 / 80 / 80 / 80 / 80 / 40
  int xCoords[] = {0, 40, 120, 200, 280, 360, 440};
  int widths[] = {40, 80, 80, 80, 80, 80, 40};
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (i == 0) { // Octave Down
      label = "<";
      if (octaveShift < 0) {
        color = alphaBlend(TFT_BLACK, TFT_CYAN, abs(octaveShift) / 5.0f);
        textColor = TFT_BLACK;
      }
      // Corner Label (v1.3)
      tft.setTextColor(TFT_WHITE);
      tft.setTextSize(1);
      char buf[4];
      sprintf(buf, "%d", octaveShift);
      tft.drawString(buf, x + 8, y + 8);
    } else if (i == 6) { // Octave Up
      label = ">";
      if (octaveShift > 0) {
        color = alphaBlend(TFT_BLACK, TFT_CYAN, abs(octaveShift) / 5.0f);
        textColor = TFT_BLACK;
      ...
      char buf[4];
      sprintf(buf, "%d", octaveShift);
      tft.drawString(buf, x + w - 8, y + 8);
    } else {
      int chordIdx = i - 1;             // 0..4
      label = chordLabels[chordIdx][0]; // Default label
      if (activeButtonIndex == i) {     // Button i is active
        label = chordLabels[chordIdx][buttonStates[i]];
    ...
    // Draw Static Outer Corner labels for Octaves (v1.3) AFTER background
    if (i == 0 || i == 6) {
      tft.setTextColor(textColor == TFT_BLACK ? TFT_BLACK : TFT_WHITE);
      tft.setTextSize(1);
      char buf[4];
      sprintf(buf, "%d", octaveShift);
      if (i == 0) {
      ...
      tft.setTextDatum(MC_DATUM); // Restore
    }
  }
}
...
void handleButtonPress(int index) {
  // index: 0=Oct-, 1=Maj, 2=Min, 3=7th, 4=Dom9, 5=Aug/Sus, 6=Oct+
  if (index == 0) { // Octave Down
    octaveShift--;
    if (octaveShift < -5)
      octaveShift = -5;
  } else if (index == 6) { // Octave Up
    octaveShift++;
    if (octaveShift > 5)
      octaveShift = 5;
  } else {
    // Chord Button (1-5)
    if (activeButtonIndex != -1 && activeButtonIndex != index) {
      // DIFFERENT BUTTON: Kill selection
      activeButtonIndex = -1;
      currentChordMask = 0xFFFF;
    } else if (activeButtonIndex == index) {
      // SAME BUTTON: Cycle bank
    ...
    if (activeButtonIndex != -1) {
      int state = buttonStates[activeButtonIndex];
      currentChordMask = chordMasks[activeButtonIndex - 1][state];
    } else {
      currentChordMask = 0xFFFF;
    }
  }
...
void setupSpeaker() {
  ...
  // CRITICAL: Stop Bluetooth to free CPU/Radio
  // DEEP CLEAN: Uninstall I2S driver to release DAC2 (GPIO 26)
  if (isBluetoothActive) {
    ...
    // Uninstall I2S driver installed by BT library
    i2s_driver_uninstall(I2S_NUM_0);
    periph_module_disable(PERIPH_I2S0_MODULE);

    delay(200); // Wait for BT stack to fully release
  ...
  // EXPLICITLY ENABLE DAC (Steal back from I2S)
  dac_output_enable(DAC_CHANNEL_2);

  // FORCE 22050Hz for Speaker Stability
  activeSampleRate = 22050;

  // SAFETY: Do not detach/reattach ISR. Just update period.
  if (timer != NULL) {
    timerAlarmWrite(timer, 1000000 / activeSampleRate, true);
    timerAlarmEnable(
        timer); // CRITICAL: Ensure timer is running if it was disabled
  }

  // CRITICAL: Enable Speaker Amp (Pin 4)
  pinMode(4, OUTPUT);
  digitalWrite(4, LOW);

  // Power Pins (from Config) + Legacy Pin 21
  if (PIN_BL != -1) {
    pinMode(PIN_BL, OUTPUT);
    digitalWrite(PIN_BL, HIGH);
  }

  // Force Pin 21 High (Legacy Amp/Mute? Critical for 2.8", maybe 3.5" too)
  pinMode(21, OUTPUT);
  digitalWrite(21, HIGH);

  if (PIN_EXT_POWER != -1) {
    pinMode(PIN_EXT_POWER, OUTPUT);
    digitalWrite(PIN_EXT_POWER, HIGH);
  }
  ...
  // Clear and Reset Pointer
  bufReadHead = 0;
  bufWriteHead = 2048; // Pre-fill offset to prevent immediate underrun race
  for (int i = 0; i < AUDIO_BUF_SIZE; i++)
    for (int i = 0; i < AUDIO_BUF_SIZE; i++)
      audioBuffer[i] = 128;

  // DAC is already initialized in setup(), nothing else needed here
}
...
void setupBluetooth(bool scanning = false) {
  ...
  // FORCE 44.1kHz for Bluetooth (Standard A2DP)
  activeSampleRate = 44100;

  // CRITICAL: DISABLE Speaker Timer Interrupt!
  // Prevents CPU starvation/conflict with BT Stack
  if (timer != NULL) {
    timerAlarmDisable(timer);
    // timerDetachInterrupt(timer); // Caused re-attach issues in Speaker Mode
  }
...
void drawEditor() {
  ...
  drawSliderControl(3 * topW, topY, topW, topH, "Range",
                    (float)activeParams.octaveRange, 1, 7, true);
...
void handleEditorTouch(int tx, int ty) {
  ...
  if (ty < topH) {
    ...
    case 3: // Octave Range
    {
      ...
      drawSliderControl(rx, ry, rw, rh, "Range", (float)v, 1, 7, true);
      lastTopPress = millis();
  ...
  // --- BOTTOM ROWS (Sliders) ---
    ...
    // Lock to the first slider touched
    if (activeSliderIdx == -1)
      activeSliderIdx = idx;
    else
      idx = activeSliderIdx;

    // Clamp and apply as before...
    idx = constrain(idx, 0, 8);
    int r_locked = idx / 3;
    int c_locked = idx % 3;

    rx = c_locked * botW;
    ry = topH + r_locked * botH;
    rw = botW;
    rh = botH;
...
void drawBootScreen() {
  ...
  tft.setTextSize(3); // Larger Title
  ...
  // Version number (now 1.1 and above instructions)
  tft.setTextColor(TFT_WHITE);
  tft.setTextSize(1);
  tft.setTextDatum(TC_DATUM);
  tft.drawString("v1.1", SCREEN_WIDTH / 2, 35);

  tft.drawString("Select Audio Output", SCREEN_WIDTH / 2, titleY + 35);

  tft.setTextDatum(MC_DATUM);

  // Split Screen (Buttons shifted down 20px)
  int btnStartY = 60;
  int btnHeight = SCREEN_HEIGHT - btnStartY - 60;

  // Left: Bluetooth
  tft.fillRect(0, btnStartY, SCREEN_WIDTH / 2, btnHeight, TFT_BLUE);
  tft.setTextColor(TFT_WHITE);
  tft.setTextSize(2);
  tft.drawString("BLUETOOTH", SCREEN_WIDTH / 4, btnStartY + btnHeight / 2);

  // Right: Speaker
  tft.fillRect(SCREEN_WIDTH / 2, btnStartY, SCREEN_WIDTH / 2, btnHeight,
               TFT_DARKGREEN);
  tft.drawString("SPEAKER", 3 * SCREEN_WIDTH / 4, btnStartY + btnHeight / 2);
...
void drawSplashScreen() {
  ...
  for (int i = 0; i < 24; i++) {
    ...
    int ly = cy;
    for (int j = 1; j <= 4; j++) {
      int nx = cx + (tx - cx) * j / 4 + random(-20, 20);
      int ny = cy + (ty - cy) * j / 4 + random(-20, 20);
      if (j == 4) {
        nx = tx;
        ny = ty;
      }
  ...
  tft.drawString("ELECTROHARP", cx + 2, cy + 2);
...
void setup() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\n--- ELECTROHARP 3.5 BOOT ---");

  // Power Pins
  if (PIN_POWER_ENABLE != -1) {
    Serial.println("Enabling Power Pin...");
  ...
  // CRITICAL: Ensure I2S is dead before touching DAC
  i2s_driver_uninstall(I2S_NUM_0);
  periph_module_disable(PERIPH_I2S0_MODULE);
  ...
  // ALLOCATE AUDIO BUFFER IN INTERNAL RAM (Critical for ISR)
  audioBuffer = (uint8_t *)heap_caps_malloc(
      AUDIO_BUF_SIZE, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if (audioBuffer == NULL) {
    Serial.println("FATAL: Failed to allocate Audio Buffer!");
  } else {
    memset((void *)audioBuffer, 128, AUDIO_BUF_SIZE);
  }
  ...
  if (!settings.touch.isCalibrated) {
    // Calibration handles its own touch init usually or we need it before?
    // Let's init touch AFTER splash/boot check to match working logic
  } else {
    // Standard Boot
    audioTarget = TARGET_BOOT;
    drawBootScreen();
    delay(500); // Allow splash to be seen
  }

  Serial.println("Initializing Shared SPI (Touch + TFT)...");
  // CYD Shared SPI Fix: Init SPI *after* TFT to prevent conflict
  SPI.begin(XPT2046_CLK, XPT2046_MISO, XPT2046_MOSI, XPT2046_CS);
  ts.begin();
  ts.setRotation(1);
  ...
  float f = 32.703f; // C1 (Starting Point)
  ...
  // Boot screen handled above to allow SPI re-init sequence

  // Start Audio Task (Core 0 to avoid Loop/UI contention on Core 1)
  xTaskCreatePinnedToCore(audioTask,  /* Function to implement the task */
                          "AudioGen", /* Name of the task */
                          10000,      /* Stack size (increased for safety) */
                          NULL,       /* Task input parameter */
                          20,         /* Priority (High/Real-Time) */
                          NULL,       /* Task handle. */
...
void triggerNote(int sIdx) {
  ...
  // Anti-Aliasing Cap: 4.8kHz for BT (44.1k), 3.2kHz for Speaker (32k/22k)
  float cap = (currentProfile == &spkProfile) ? 3200.0f : 4800.0f;
  if (freq > cap)
    freq = cap;
  ...
  */
  ...
  // Global Octave Shift handled by getGlobalNoteIndex now
  // We only handle the extra Sine boost here if needed
  if (extraOctave > 0) {
    for (int k = 0; k < extraOctave; k++)
      freq *= 2.0f;
  }
  ...
  // Frequency Clamping (C1 to C10-ish)
  if (freq < 32.7f)
    freq = 32.7f; // Clamp to C1
  if (freq > 15500.0f)
    freq = 15500.0f; // Clamp below Nyquist (C10 is ~16.7k)
  ...
  // Find Voice
  int vIdx = -1;
  // Dynamic Governor Check (Tiered)
  // Max Polyphony is determined by the Load Governor in fillAudioBuffer
  int effectiveLimit = maxPolyphony;
  ...
  // 1. Try to find Inactive (if under limit)
  ...
  // 2. Stealing Logic (If capped OR no empty slots)
  if (vIdx == -1) {
  ...
  voices[vIdx].trigger(freq, sIdx, currentWaveform, globalPulseWidth,
                       activeParams.attackTime, activeParams.releaseTime * 0.3f,
                       0.7f, activeParams.releaseTime);
  voices[vIdx].isSparkle = false;
  voices[vIdx].isLatchedArp = false;
...
void fireArp(int octaveOffset, bool latched) {
  ...
  if (latched) {
    ...
    int effectiveOctave = latched
                              ? (latchedOctaveShift + 1)
                              : octaveShift; // Lowered to +1 per user request
...
void loop() {
  ...
  if (arpMode != ARP_OFF && arpLatch) {
    ...
    if (millis() - lastArpClock > interval) {
      int homeOffset = getHomeOffset();
      int lowestOnScreen = homeOffset + (latchedOctaveShift * 12);
      int lowestRootOnScreen = (lowestOnScreen / 12) * 12;
      if (lowestRootOnScreen < lowestOnScreen)
        lowestRootOnScreen += 12;
      int arpRegister = max(lowestRootOnScreen, 24); // C3 or higher

      fireArp(arpRegister, true); // Latched Call
      lastArpClock = millis();
    }
  }
  ...
  if (audioTarget == TARGET_BOOT || audioTarget == TARGET_CALIBRATION_1 ||
    ...
    if (audioTarget == TARGET_CALIBRATION_2) {
      if (ts.touched()) {
        ...
        if (p.z > 500) {
          ...
          // Check Swap? (Assume Standard for now, or detect if min > max)
          ...
          // Add margin? Raw values are usually exact.
          // Let's rely on map() to handle it.
    ...
    static bool panicWaitingForRelease = false; // New state for panic
    ...
    if (ts.touched()) {
      ...
      if (p.z > 200) {
        ...
        if (!isBootTouching) {
          ...
          if (millis() - bootTouchStartLocal > 3000 &&
              !panicWaitingForRelease) {
            panicWaitingForRelease = true; // Set flag, wait for release
            // Don't change audioTarget yet, wait for release
          }
        }
      }
    } else {
      ...
      if (isBootTouching) {
        ...
        // Use raw coordinates or default calibration with INVERSION fix
        int touchX = map(lastBootTouch.x, settings.touch.minX,
                         settings.touch.maxX, SCREEN_WIDTH, 0); // Inverted X
        int touchY = map(lastBootTouch.y, settings.touch.minY,
                         settings.touch.maxY, SCREEN_HEIGHT, 0); // Inverted Y
  ...
  if (audioTarget == TARGET_BT_SELECT) {
    ...
    if (ts.touched()) {
      ...
      if (p.z > 400) {
        // Map Touch
        // Map Touch (Inverted for correct orientation)
        int touchX =
            map(p.x, settings.touch.minX, settings.touch.maxX, SCREEN_WIDTH, 0);
        int touchY = map(p.y, settings.touch.minY, settings.touch.maxY,
                         SCREEN_HEIGHT, 0);

        // Use mapped variables directly (removing redundant standardized vars
        // below)
        int mappedTouchX = constrain(touchX, 0, SCREEN_WIDTH);
        int mappedTouchY = constrain(touchY, 0, SCREEN_HEIGHT);

        int cols = 3;
        int rows = 2;
        int col = mappedTouchX / (SCREEN_WIDTH / cols);
        int row = mappedTouchY / (SCREEN_HEIGHT / rows);
        ...
        if (idx == 5) { // REFRESH
          Serial.println("Refreshing Scan (List Cleared)...");
          foundDeviceCount = 0;
          targetSSID[0] = '\0';
          drawBTSelectScreen(idx); // Keep Highlight?
          // DO NOT call end/start! Scan continues in background.
        } else {
          // DEVICE SELECT
          if (idx < foundDeviceCount) {
  ...
  if (audioTarget == TARGET_SPEAKER || audioTarget == TARGET_BLUETOOTH ||
    ...
    if (audioTarget == TARGET_SPEAKER) {
      // fillAudioBuffer(); // HANDLED BY FREERTOS TASK
    }
    ...
    if (currentMode == MODE_EDIT) {
      ...
      if (!ts.touched()) {
        ...
        if (millis() - releaseDebounceStart > 50) {
          ...
          activeSliderIdx = -1; // Reset stickiness on release
        }
      } else {
        ...
        if (!editorInputBlocked) {
          TS_Point p = ts.getPoint();
          if (p.z > 400) {
            int tx = constrain(map(p.x, settings.touch.minX,
                                   settings.touch.maxX, SCREEN_WIDTH - 1, 0),
                               0, SCREEN_WIDTH - 1);
            int ty = constrain(map(p.y, settings.touch.minY,
                                   settings.touch.maxY, SCREEN_HEIGHT - 1, 0),
                               0, SCREEN_HEIGHT - 1);
    ...
    if (millis() - heartbeat > 2000) {
      Serial.printf("ISR: %u | R: %d | W: %d | Fill: %u us | Free: %d\n",
                    isrCount, bufReadHead, bufWriteHead, lastFillDuration,
                    (bufReadHead - bufWriteHead + AUDIO_BUF_SIZE - 1) %
                        AUDIO_BUF_SIZE);
      heartbeat = millis();
    }
    ...
    if (ts.touched()) {
      ...
      int tx = constrain(
          ...
          0, SCREEN_WIDTH);
      int ty = constrain(
          map(p.y, settings.touch.minY, settings.touch.maxY, SCREEN_HEIGHT, 0),
          0, SCREEN_HEIGHT);
      ...
      // --- TOUCH DISPATCH WITH BUFFERS ---
      // ty Ranges: Top Row (0-70), Strings (80-200), Vol (210-250), Chords
      // (255-320)
      ...
      if (ty < 60) {
        ...
        // Touch Target Calibration:
        // Delay (0): shift boundary left (80 -> 70)
        // Wave (5): shift boundary right (400 -> 415)
        ...
        } else if (btnIdx == 1) { // Drive
          if (drivePressStart == 0) {
            fxDrive = !fxDrive;
            drawFXButtons();
            drivePressStart = millis();
          }
        } else if (btnIdx == 2) { // Trem
          if (tremPressStart == 0) {
            fxTrem = !fxTrem;
            drawFXButtons();
            tremPressStart = millis();
          }
        } else if (btnIdx == 3) { // LFO
          if (lfoPressStart == 0) {
            fxLFO = !fxLFO;
            drawFXButtons();
            lfoPressStart = millis();
          }
        } else if (btnIdx == 4) { // Arp
          if (arpPressStart == 0)
            arpPressStart = millis();
          if (millis() - arpPressStart > 250 && !waitForArpRelease) {
            arpLatch = !arpLatch;
            if (arpLatch)
        ...
        } else if (btnIdx == 5) { // Wave
          if (wavePressStart == 0)
            wavePressStart = millis();

          // Reliable Long Press (v1.3)
          if (millis() - wavePressStart > 600) {
      ...
      // Strings: Active 63 to 177 (3px buffer)
      else if (ty >= 63 && ty < 177) {
        int numStrings = activeParams.octaveRange * 12 + 1;
        int sIdx = (tx * numStrings) / SCREEN_WIDTH;
        if (sIdx != lastTouchedString) {
          ...
          if (arpMode != ARP_OFF && !arpLatch) {
            if (currentChordMask & (1 << (getGlobalNoteIndex(sIdx) % 12))) {
              int gIdx = getGlobalNoteIndex(sIdx);
              fireArp((gIdx / 12) * 12);
            }
          } else {
            triggerNote(sIdx);
      ...
      // Vol/Trans: Active 183 to 244
      else if (ty >= 183 && ty < 244) {
        int volW = (SCREEN_WIDTH * 3) / 4;
        if (tx < volW) {
          float volNormalized = (float)tx / (float)volW;
          masterVolume = volNormalized * volNormalized; // Quadratic (v1.3)
          drawVolumeSlider();
        } else if (tx > volW + 6) { // 6px buffer after Volume (v1.3)
          static uint32_t lastTransTime = 0;
          if (millis() - lastTransTime > 400) {
      ...
      // Presets: Active >= 250
      else if (ty >= 253) { // 3px buffer from 250
        // Find button based on xCoords and widths + 3px padding
        int xCoords[] = {0, 40, 120, 200, 280, 360, 440};
        int widths[] = {40, 80, 80, 80, 80, 80, 40};
        int btnIdx = -1;
        for (int i = 0; i < 7; i++) {
          if (tx >= xCoords[i] + 3 && tx < xCoords[i] + widths[i] - 3) {
            btnIdx = i;
            break;
          }
        }
        ...
        if (btnIdx != -1 && millis() - lastBtnPress > 300) {
          handleButtonPress(btnIdx);
          lastBtnPress = millis();
        }
      }
    } else { // No Touch Logic (On Release)
      if (lastTouchedString != -1) {
        // Release any voices tied to the last strummed string
        for (int v = 0; v < MAX_VOICES; v++) {
          if (voices[v].active && voices[v].held && !voices[v].isLatchedArp)
            voices[v].release();
        }
      ...
      lastTouchedString = -1;
      waitForArpRelease = false;
      activeSliderIdx = -1; // Reset Editor drag lock

      if (wavePressStart > 0) {
        if (millis() - wavePressStart < 250) { // TAP
          int nextW = (int)currentWaveform + 1;
          if (nextW > 3)
            nextW = 0;
          selectWaveform(nextW);
          drawWaveButton();
      ...
      if (arpPressStart > 0) {
        if (millis() - arpPressStart < 250) { // TAP
          if (arpLatch) {
            arpLatch = false;
            releaseLatchedVoices();
          } else {
            arpMode = (ArpMode)((int)arpMode + 1);
            if (arpMode > ARP_SPARKLE2)
              arpMode = ARP_OFF;
            updateActiveNotes();
          }
          drawArpButton();
      ...
      if (delayPressStart > 0) { // TAP
        if (millis() - delayPressStart < 500) {
          delayMode = (delayMode + 1) % 5;
          drawDelayButton();
        }
        delayPressStart = 0;
      }
      drivePressStart = tremPressStart = lfoPressStart = 0;
      lastChordBtn = -1; // Reset chord button tracking on release
    }
  }
}
