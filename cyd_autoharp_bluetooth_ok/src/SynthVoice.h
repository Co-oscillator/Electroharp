...
public:
  ...
  float IRAM_ATTR getSample(float pitchMod) {
    ...
    if (waveform == WAVE_SAW) {
      ...
      sample = raw - dcOffset;
      sample *=
          0.50f; // Reduced to prevent clipping on narrow pulses (1.9x peak)
    } else if (waveform == WAVE_SINE) {
      // High quality sine (No Wavefolding)
      sample = sin(phase * 6.283185307f);
      sample *=
          0.85f; // Reduced Gain (was 0.95f) to prevent clipping / masking delay
      ...
      sample = tri;
      sample *= 0.80f; // Boosted Volume (was 0.30f)
    }
