...
void updateStringVisuals() {
  ...
  for (int i = 0; i < numStrings; i++) {
    ...
    // 2. Decay Energy
    if (stringEnergy[stringIdx] > 0.01f) {
      stringEnergy[stringIdx] *= 0.94f;
    } else {
      stringEnergy[stringIdx] = 0.0f;
    }

    // 3. Determine Color
    bool isBlack = isBlackKey(stringIdx);
    uint16_t baseColor = isBlack ? COLOR_STRING_BLACK : COLOR_STRING_WHITE;
    uint16_t activeColor = TFT_ORANGE;

    uint16_t currentColor =
        stringEnergy[stringIdx] > 0
            ? alphaBlend(baseColor, activeColor, stringEnergy[stringIdx])
            : baseColor;

    // 4. Draw ONLY if color changed
    if (currentColor != lastStringColor[stringIdx]) {
      int drawY = 60;
      int drawH = 140 - drawY;
      ...
      // Check absolute pitch relative to C2 (index 0)
      // If (stringIdx + rootNote) % 12 == 0, it's a Root.
      // Actually simpler: if stringIdx % 12 == 0, it's a C.
      // User likely wants ROOT labeled.
      int noteVal = (stringIdx + rootNote) % 12;
      ...
      if (noteVal == 0) {
        ...
        tft.setTextSize(1);
        tft.drawString(noteNames[0], centerX, circleY); // Always C/Root
      }

      lastStringColor[stringIdx] = currentColor;
    }
  }
}
