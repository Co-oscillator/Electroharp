...
- **Arp**: 
    ...
    - **Long Press (250ms)**: Toggles **Arp Latch**. When latched, a single tap will un-latch the current pattern. Arpeggios now release naturally when unlatched.
- **Wave**: 
    - **Tap**: Cycles Waveforms (Saw, Square, Sine, Triangle).
    - **Long Press (250ms)**: Enters the **Editor Screen**.
...
- **Visuals**: Primary notes are Pale Cream; accidentals are Sky Blue. Root notes (C) are highlighted with circles and labels.
- **Centering**: The visible string area is intelligently centered on Middle C (C4), ensuring a musical range regardless of the selected octave span.
...
- **Range**: Adjust the harp span from 1 to 8 octaves. The engine now supports a massive **C1 (32Hz)** to **C10 (15kHz)** range with +/- 5 Octave Shifts.
...
- **Polyphony**: 8 Voices with priority stealing for latched arps.
- **Audio Config**: The advanced audio routing menu is currently a work-in-progress, labeled **"(exp)"**.
