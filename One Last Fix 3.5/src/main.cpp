...
int getGlobalNoteIndex(int sIdx) {
  int idx = sIdx + getHomeOffset() + (octaveShift * 12);
...
float generateMixedSample(float pitchMod, float pwMod, float filterMod,
  ...
  // Apply Filter Cutoff (LFO modulation removed per user request)
  float f = svf_f;
  if (f > 0.85f)
  ...
  if (currentWaveform == WAVE_SAW) {
    ...
    q = q * 3.0f; // Triple damping for Sine to prevent "explosion"
  }

  // Speaker Profile Safety: Extra damping for high notes/resonance
...
void fillAudioBuffer() {
  uint32_t startT = micros(); // START PROFILE
  ...
  while (samplesToFill > 0) {
    ...
    // Depth: globalLfoDepth (LDR) * activeParams.lfoDepth
    float effectiveDepth = activeParams.lfoDepth * globalLfoDepth; // 0..1

    if (activeParams.lfoTarget == TARGET_PITCH) {
      pitchMod = lfoVal * effectiveDepth * 0.1f;
    } else if (activeParams.lfoTarget == TARGET_FILTER) {
    ...
    if (delayMode > 0 && delayBuffer != 0) {
      int delayMs = delayMode * 300;
      int delaySamples = (activeSampleRate / DELAY_DOWNSAMPLE * delayMs) / 1000;
      if (delaySamples >= MAX_DELAY_LEN)
        delaySamples = MAX_DELAY_LEN - 1;
      int readPos = (delayHead - delaySamples + MAX_DELAY_LEN) % MAX_DELAY_LEN;
      delayed = (float)delayBuffer[readPos] * 3.3333e-5f;
    }
    ...
    // Audio Test Tone
    if (isAudioTestRunning) {
      static float testPhase = 0;
      testPhase += 2.0f * PI * 440.0f / (float)activeSampleRate;
      if (testPhase > 2.0f * PI)
        testPhase -= 2.0f * PI;
      s += sin(testPhase) * 0.5f;
    }
    ...
    // Convert to 8-bit DAC (0-255)
    uint8_t out = (uint8_t)((s * 127.0f) + 128.0f);

    // Write sample
    audioBuffer[bufWriteHead] = out;
    bufWriteHead = nextWrite;
    samplesToFill--;
  }

  // END PROFILE & GOVERNOR Logic
  uint32_t elapsed = micros() - startT;
  // 256 samples @ 44.1k = ~5800us budget.
  // @ 32k = ~8000us budget.
  ...
  if (millis() - lastLoadCheck > 50) { // Fast check (50ms)
    // Tiered Stealing Thresholds (Rev 21)
    if (cpuLoad > 0.90f) {
      maxPolyphony = 6; // CRITICAL EMERGENCY
    } else if (cpuLoad > 0.80f) {
      if (maxPolyphony > 10)
        maxPolyphony = 10; // Aggressive Steps
    } else if (cpuLoad > 0.70f) {
      if (maxPolyphony > 16)
        maxPolyphony = 16; // Moderate
    } else if (cpuLoad < 0.60f) {
      // Recovery
      if (maxPolyphony < MAX_VOICES)
        maxPolyphony++;
...
void updateStringVisuals() {
  ...
  for (int i = 0; i < numStrings; i++) {
    ...
    bool isBlack = isBlackKey(globalIdx % 12);
  ...
  if (circlesNeedRedraw) {
    for (int i = 0; i < numStrings; i++) {
      ...
      if (globalIdx % 12 == 0) {
        int centerX =
            (i * SCREEN_WIDTH) / numStrings + (SCREEN_WIDTH / (numStrings * 2));
        int circleY = 80 + 12; // Adjusted for 3.5" offset
        int radius = 10;
...
void setupSpeaker() {
  ...
  // SWITCH TO 32kHz for Speaker (To fix "squeak" / stability)
  ...
  // Enable Amplifier (FM8002E Shutdown is Pin 4 - Active High Shutdown)
  // Power Pins
  ...
  // Speaker Enable (Initialized as OFF)
  if (SPEAKER_ENABLE_PIN != -1) {
...
void triggerNote(int sIdx) {
  ...
  // 2. Try to steal oldest active (non-held) UNLATCHED voice
  if (vIdx == -1) {
    for (int v = 0; v < MAX_VOICES; v++) {
      // Primitive stealing: Just find one that isn't a Latched Arp and isn't
      // Held (Release phase)
      if (!voices[v].held && !voices[v].isLatchedArp) {
        vIdx = v; // Good enough
        break;
      }
    }
  }
  // 3. Steal oldest UNLATCHED (even if held)
  if (vIdx == -1) {
    for (int v = 0; v < MAX_VOICES; v++) {
  ...
  // 4. Fallback: Steal 0 (Even if latched... gotta play something)
  if (vIdx == -1)
    vIdx = 0;
...
void loop() {
  ...
  if (audioTarget == TARGET_BT_SELECT) {
    ...
    if (ts.touched()) {
      ...
      if (p.z > 400) {
        // Map Touch
        int touchX = map(p.x, settings.touch.minX, settings.touch.maxX, 0, 320);
        int touchY = map(
            p.y, 450, 3700, 0,
            240); // This mapping seems off, should use settings.touch.minY/maxY

        // Standardize touch variables
        int mappedTouchX = constrain(
            map(p.x, settings.touch.minX, settings.touch.maxX, 0, SCREEN_WIDTH),
            0, SCREEN_WIDTH);
        int mappedTouchY = constrain(map(p.y, settings.touch.minY,
