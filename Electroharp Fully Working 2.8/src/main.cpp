...
int32_t bt_data_stream_callback(Frame *data, int32_t len) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    float wobble =
        (sin(wowPhase) + sin(flutterPhase)) * 0.0015f; // reduced wobble
...
void updateStringVisuals() {
  ...
  for (int i = 0; i < numStrings; i++) {
    ...
    if (stringEnergy[i] > 0.01f) {
      stringEnergy[i] *= 0.94f; // Slightly faster decay for thin strings
    } else {
      stringEnergy[i] = 0.0f;
    }
    ...
    if (currentColor != lastStringColor[i]) {
      ...
      // Root Note Circles (Every C note)
      if (i % 12 == 0) {
        int circleY = drawY + 12;
        int radius = 10;

        // Match circle to string color
        tft.fillCircle(centerX, circleY, radius, currentColor);
        tft.drawCircle(centerX, circleY, radius, TFT_DARKGREY);

        // Label inside circle: Contrast text
        // Yellow/Blue strings are light, so black text is best
