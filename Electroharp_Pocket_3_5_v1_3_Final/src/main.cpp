...
void drawVolumeSlider() {
  ...
  tft.fillRect(2, y + 2, fillW, h - 4, TFT_NAVY);
...
void updateButtonVisuals() {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    // Draw Octave Number Overlay
    if ((i == 0 && octaveShift < 0) || (i == 6 && octaveShift > 0)) {
      tft.setTextSize(1);
      tft.setTextColor(textColor == TFT_BLACK ? TFT_BLACK : TFT_WHITE);
      String s = String(abs(octaveShift));
      if (i == 0) {
        tft.setTextDatum(BL_DATUM); // Corner
        tft.drawString(s, x + 4, y + h - 4);
      } else {
        tft.setTextDatum(BR_DATUM); // Corner
        tft.drawString(s, x + w - 4, y + h - 4);
      }
  ...
  if (audioTarget == TARGET_SPEAKER || audioTarget == TARGET_BLUETOOTH ||
    ...
    if (ts.touched()) {
      ...
      // Vol/Trans: 185-235
        ...
        int volW = (SCREEN_WIDTH * 3) / 4;
        if (tx < volW - 10) { // 10px buffer before Transpose
          masterVolume = (float)tx / (float)volW;
          drawVolumeSlider();
        } else if (tx > volW + 10) { // 10px buffer after Volume
      ...
      // Chord Row: 250-320
        ...
        int xCoords[] = {0,   32,  120, 200,
                         280, 360, 448};             // Corrected boundaries
        int widths[] = {32, 88, 80, 80, 80, 88, 32}; // Adjusted widths
        int pressedIdx = -1;
        for (int i = 0; i < 7; i++) {
          int buffer = (i == 0 || i == 6) ? 0 : 1;
          if (tx >= xCoords[i] + buffer &&
              tx < xCoords[i] + widths[i] - buffer) {
            pressedIdx = i;
            break;
          }
        }

        if (pressedIdx != -1 && pressedIdx != lastChordBtn) {
          handleButtonPress(pressedIdx);
          lastChordBtn = pressedIdx;
        }
      }
