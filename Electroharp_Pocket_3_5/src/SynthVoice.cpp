...
void SynthVoice::trigger(float freq, int noteIdx, Waveform wave, float pw,
  ...
  // Fallback for linkage issues
  float fs = (activeSampleRate > 0) ? (float)activeSampleRate : 22050.0f;

  attackRate = 1.0f / (attack * fs);
  decayRate = 1.0f / (decay * fs);
  releaseRate = 1.0f / (release * fs);
  sustainLvl = sustain;

  // Recalculate phase increment with safe rate
