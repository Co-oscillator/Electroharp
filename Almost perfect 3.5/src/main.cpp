...
void fillAudioBuffer() {
  ...
  while (samplesToFill > 0) {
    ...
    if (fxLFO) {
      // Use User Depth (Modulated by LDR via globalLfoDepth)
      ...
      } else if (activeParams.lfoTarget == TARGET_LFO_DEPTH) {
        // Self-Modulation (LFO modulates its own depth?)
        // Or LDR modulates Depth? "LDR target list should contain LFO Depth".
        // This block is LFO TARGET. The user said "LDR target list".
        // So LDR modulates Depth.
        // Wait, does LFO modulate Depth? "LFOD" option is for LDR.
        // If user selects "LFOD" in LFO Target, it means LFO modulates LFO
        // Depth? That's complex (AM). Let's support it if easy. depth += lfoVal
        // * depth * 0.5f? Recursive. Let's stick to LDR for now as explicitly
        // requested.
      }
    ...
    // --- LDR Modulation ---
    // Read LDR (Global or passed?) - ADC is slow, usually read in loop().
    // We need a thread-safe way to get LDR.
    // Assuming `globalLdrValue` exists or we interpret `activeParams.ldrTarget`
    // here? Actually, LDR is likely applied to parameters in the loop. BUT user
    // wants audio-rate smooth modulation if possible? LDR is slow physically.
    // Loop update is fine.

    // Check `main loop` for LDR mapping.
    // ...
    // Looking at previous `loop` code (not visible here), it handled LDR.
    // I need to check `loop` logic.
    ...
    // "Speaker Silence": Use neutral 128 if no voices active to stop buzz
    // (activeCount tracks voices)
    int dacValue = 128;
    int voiceActiveCount = 0;
    for (int v = 0; v < MAX_VOICES; v++)
      if (voices[v].active)
        voiceActiveCount++;

    if (voiceActiveCount > 0 || delayMode > 0) {
      dacValue = (int)((s + 1.0f) * 127.5f);
      if (dacValue < 0)
        dacValue = 0;
      if (dacValue > 255)
        dacValue = 255;
